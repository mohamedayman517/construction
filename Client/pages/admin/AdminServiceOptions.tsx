...
import { getAdminServiceOptions, saveAdminServiceOptions } from '../../lib/adminOptions';
...
export default function AdminServiceOptions(props: Partial<RouteContext>) {
  ...
  useEffect(() => {
    const opts = getAdminServiceOptions();
    setCategories(opts.categories);
    const onUpdate = () => setCategories(getAdminServiceOptions().categories);
  ...
  const addItem = () => {
    ...
    setCategories(next);
    saveAdminServiceOptions({ categories: next });
    setNewItem('');
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-2xl font-bold mb-2">{locale==='ar' ? 'خيارات الخدمات (للوحة الأدمن)' : 'Service Options (Admin)'}</h1>
        <p className="text-muted-foreground mb-6">{locale==='ar' ? 'أضف/احذف أنواع الخدمات' : 'Add/remove service categories'}</p>

        <Card>
          <CardHeader>
            <CardTitle>{locale==='ar' ? 'أنواع الخدمات' : 'Service Categories'}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex gap-2 mb-4">
              <Input
                placeholder={locale==='ar' ? 'نوع الخدمة (عربي)' : 'Service category (Arabic label)'}
                value={newItem}
